...
public class CommonEvents 
{
	 ...
	 * @throws FindFailed
	 */
	public CommonEvents sikuliClick(String imagePath) throws FindFailed
	{
		Pattern openButton = new Pattern(imagePath).similar(0.1d);
		new Screen().wait(imagePath, 10)
					.click(openButton);
		return new CommonEvents(driver);
	}
	...
	* @param xOffset - {@link Integer} type: The value should be shifted in the horizontal axis
	...
	{
		int count=1;
		long maxIterationCount = maxTimeout*2;
		boolean flag = false;
		long totalCount = 0;
		do {
			Thread.sleep(500);
		...
		}while(count<maxIterationCount && !flag);
		if(count==maxIterationCount && flag==false)
